...
class Policy:
    ...
    def _build_net(self):
        ...
        with tf.variable_scope('eval_net'):
            ...
            c1 = tf.layers.conv2d(input, filter=(8,8),strides=(4,4),kernel_initializer=w_initializer,
                                 bias_initializer=b_initializer)

            p1 = tf.layers.max_pooling2d(c1, pool_size=(2,2),strides=(1,1), padding='valid')

            c2 = tf.layers.conv2d(p1, filter=(8,8),strides=(4,4),kernel_initializer=w_initializer,
                                 bias_initializer=b_initializer)

            p2 = tf.layers.max_pooling2d(c2, pool_size=(2,2),strides=(1,1), padding='valid')
            
            #プーリング層
            f = tf.layers.flatten(p2,name=None)


            e1 = tf.layers.dense(f, 30, tf.nn.relu, kernel_initializer=w_initializer,
